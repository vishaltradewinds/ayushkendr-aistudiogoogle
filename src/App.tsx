...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (query.get('checkout') === 'success') {
      ...
      alert('Payment was cancelled. Your orders are pending.');
      setActiveTab('orders');
      // Clean up URL
      window.history.replaceState({}, document.title, window.location.pathname);
    }
  ...
  const filteredProducts = products.filter(p => {
    ...
    const matchesBrand = brandFilter === 'All' || p.vendor_name === brandFilter;
    
    const matchesPrice = p.price >= priceRangeFilter[0] && p.price <= priceRangeFilter[1];
    
    const isAyush = p.category.toLowerCase().includes('ayush') || 
                    p.category.toLowerCase().includes('ayurveda') || 
                    p.category.toLowerCase().includes('yoga') || 
                    p.category.toLowerCase().includes('unani') || 
                    p.category.toLowerCase().includes('homeopathy');
    
    const matchesType = typeFilter === 'All' || 
                        (typeFilter === 'AYUSH' && isAyush) || 
                        (typeFilter === 'Modern' && !isAyush);

    return matchesSearch && matchesCategory && matchesAvailability && matchesRating && matchesBrand && matchesPrice && matchesType;
  });
    ...
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-6 relative">
        ...
        >
          ...
          <div className="mt-6">
            <button 
              onClick={() => alert("Google Sign-In integration requires a valid Client ID. Please configure GOOGLE_CLIENT_ID in the environment variables.")}
              className="w-full flex items-center justify-center gap-3 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 py-3.5 rounded-xl font-bold transition-colors shadow-sm"
            >
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      ...
                      <option value="AYUSH">AYUSH Products</option>
                      <option value="Modern">Modern Medical</option>
                    </select>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <select 
                      ...
                      onChange={(e) => {
                        ...
                        if (val === 'All') setPriceRangeFilter([0, 1000000]);
                        else {
